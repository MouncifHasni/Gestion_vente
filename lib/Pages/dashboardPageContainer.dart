...
class DashboardPageContainer extends StatefulWidget {
  @override
  _DashboardPageContainer createState() => _DashboardPageContainer();
}

class _DashboardPageContainer extends State<DashboardPageContainer> {
  String _clientsCount = '';
  String _fournisseursCount = '';
  String _ventesCount = '';
  List<FactureJson> _listfactures;
  List<ChartModel> _listrevenue = [];
  double _maxRevenue = 40000;
  ...
  void initState() {
    ...
    getClientsCount().then((value) {
      setState(() {
        _clientsCount = value;
      });
    });
    getFournisseursCount().then((value) {
      setState(() {
        _fournisseursCount = value;
      });
    });
    getVentesCount().then((value) {
      setState(() {
        _ventesCount = value;
      });
    });
    ...
    return Container(
      ...
      children: [
        ...
        Flexible(
          child: Row(
            children: [
              _buildStateCard("Clients", _clientsCount, Colors.blue),
              _buildStateCard("Ventes", _ventesCount, Colors.green),
              _buildStateCard(
                  "Fournisseurs", _fournisseursCount, Colors.orange),
            ],
          ),
        ),
        SizedBox(
  ...
  Flexible _buildStateCard(String title, String value, MaterialColor color) {
    return Flexible(
      child: Container(
